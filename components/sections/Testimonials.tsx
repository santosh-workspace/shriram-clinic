...
/** Shared SVG sprite — defined once, referenced via <use>. */
function SvgSprite() {
  return (
    <svg className="hidden" aria-hidden="true">
      <defs>
        <symbol id="icon-star" viewBox="0 0 24 24">
          <path fill="currentColor" d="M12 17.3l-6.16 3.7 1.64-7.03L2 9.24l7.19-.61L12 2l2.81 6.63 7.19.61-5.48 4.73 1.64 7.03z" />
        </symbol>
  ...
  return (
    <span className="inline-flex gap-0.5" aria-hidden="true">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} width={size} height={size} className="text-[#F5A623]">
          <use href="#icon-star" />
        </svg>
      ))}
    </span>
